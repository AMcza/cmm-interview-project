...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(StringUtils.isBlank(answer), ErrorCode.PARAMS_ERROR,"答案不能为空");
        }
        // 修改数据时，有参数则校验
        // todo 补充校验规则
        if (StringUtils.isNotBlank(title)) {
            ThrowUtils.throwIf(title.length() > 80, ErrorCode.PARAMS_ERROR, "标题过长");
            ThrowUtils.throwIf(tags.length()>100,ErrorCode.PARAMS_ERROR,"标签过长");
        }
    }
    ...
    public Page<Question> searchFromEs(QuestionQueryRequest questionQueryRequest) {
        ...
        if(searchHits.hasSearchHits()){
            ...
            for(SearchHit<QuestionEsDTO> searchHit:searchHitsList){
                resourceList.add(questionMapperStruct.dtoToObj(searchHit.getContent()));
            }
        }
